#!/usr/bin/env python3
"""
Velour Live Complete Historical Scraper
Scrapes ALL available historical data from 2006-2026 using the correct parser
"""
...
class VelourCompleteHistoricalScraper:
    def __init__(self):
    ...
    def scrape_all_historical_data(self, start_year: int = 2006, end_year: int = 2026) -> List[Dict]:
        ...
        print(f"=== Velour Live Complete Historical Data Scraper ===")
        ...
        for year in range(start_year, end_year + 1):
            ...
            for month in range(1, 13):
                ...
                try:
                    ...
                    # Save progress every 100 shows
                    if len(all_shows) % 100 == 0 and len(all_shows) > 0:
    ...
    def save_final_dataset(self, filename: str = None) -> str:
        ...
        print(f"Complete historical dataset saved to: {csv_file}")
        ...
        print(f"Complete historical dataset saved to: {json_file}")
        
        # Save as TSV
        tsv_file = os.path.join(self.project_root, 'data', 'exports', f"{filename}.tsv")
        df.to_csv(tsv_file, index=False, sep='\t')
        print(f"Complete historical dataset saved to: {tsv_file}")
...
def main():
    scraper = VelourCompleteHistoricalScraper()
    
    try:
        print("Starting comprehensive historical data collection...")
        print("This will collect 20 years of Velour Live show data!")
        print("Starting with a test run on recent years...\n")
        
        # Start with a smaller range to test first
        print("Testing with 2024-2025...")
        test_shows = scraper.scrape_all_historical_data(start_year=2024, end_year=2025)
        
        if test_shows:
            print(f"\nTest successful! Found {len(test_shows)} shows from 2024-2025")
            
            # Ask if we should continue with full historical data
            print(f"\nProceeding with full historical data collection (2006-2026)...")
            print("This will take a while - collecting 20 years of data!")
